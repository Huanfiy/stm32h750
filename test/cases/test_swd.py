#!/usr/bin/env python3
"""SWD closed-loop test.

Resets the target via J-Link, then halts and verifies:
- PC sits inside the QSPI XIP window (0x90000000+), proving the bootloader
  successfully jumped to the app and that the app is executing from external
  flash (no fault, no spin in internal-flash halt loop).
- the core is not STUCK in an exception. A single halt snapshot may
  legitimately land inside a transient ISR (SysTick/DMA — a healthy system
  spends a few percent of its time there), so IPSR != 0 triggers a
  resume + re-halt: only consistently exceptional samples FAIL. A real
  HardFault/assert-in-handler stays pinned and still fails every retry.
"""

from __future__ import annotations

import sys
from pathlib import Path

sys.path.insert(0, str(Path(__file__).resolve().parents[1]))
from lib import jlink  # noqa: E402

EXIT_PASS, EXIT_FAIL, EXIT_SKIP = 0, 1, 77

QSPI_XIP_BASE = 0x90000000
QSPI_XIP_END = 0x90800000  # 8 MB W25Q64
ISR_RESAMPLES = 3  # extra halt snapshots before declaring "stuck in exception"


def main() -> int:
    if not jlink.have_jlink():
        print("SKIP: JLinkExe not in PATH")
        return EXIT_SKIP

    try:
        # One J-Link session: reset+run, settle in-probe, halt, snapshot, resume.
        regs = jlink.halt_and_regs(pre_cmds=["r", "g", "Sleep 1000"])
    except jlink.JLinkUnavailable as exc:
        print(f"SKIP: J-Link unreachable: {exc}")
        return EXIT_SKIP

    ipsr = None
    for attempt in range(1 + ISR_RESAMPLES):
        if attempt > 0:
            # Target was resumed at the end of the previous session; let it
            # run a little and sample again — a transient ISR will have
            # returned, a stuck exception stays pinned.
            regs = jlink.halt_and_regs(pre_cmds=["Sleep 200"])

        pc = regs.get("PC") or regs.get("R15")
        xpsr = regs.get("XPSR") or regs.get("xPSR")
        if pc is None or xpsr is None:
            print(f"FAIL: missing PC/XPSR in {sorted(regs)}")
            return EXIT_FAIL

        ipsr = xpsr & 0x1FF
        print(f"PC=0x{pc:08X}  XPSR=0x{xpsr:08X}  IPSR={ipsr}")

        if not (QSPI_XIP_BASE <= pc < QSPI_XIP_END):
            print(f"FAIL: PC 0x{pc:08X} not in QSPI XIP window")
            return EXIT_FAIL
        if ipsr == 0:
            print("PASS: app running from QSPI, no exception")
            return EXIT_PASS
        print(f"halted inside ISR (IPSR={ipsr}), resampling "
              f"{attempt + 1}/{ISR_RESAMPLES}...")

    print(f"FAIL: IPSR={ipsr} across {1 + ISR_RESAMPLES} samples — "
          f"core is stuck in an exception")
    return EXIT_FAIL


if __name__ == "__main__":
    sys.exit(main())
